...
test("assembly extensions render in both accepted spellings", async () => {
  ...
  circuit.add(
    <assembly.device name="D1">
      ...
      {/* Each insert-backed mount requires a bolt. */}
...
test("a screw carries its thread and head without an authored length", async () => {
  const { circuit } = getTestFixture()
  ...
  circuit.add(
    <assembly.device name="D1">
      <board name="B1" width="40mm" height="24mm" routingDisabled>
        <hole name="H2" pcbX={15} pcbY={-8} diameter="3.2mm">
          <assembly.screw thread="m3" head="panhead" />
        </hole>
      </board>
      {/* a self-tapping M3 needs a deeper boss than the 4mm default standoff */}
      <enclosure.fdm.box name="EN1" boardRef=".B1" standoffHeight="8mm" />
    </assembly.device>,
  )
  await circuit.renderUntilSettled()
  ...
  expect(screw!._parsedProps.head).toBe("panhead")
